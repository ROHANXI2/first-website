...
// @desc    Test registration (for development/testing only)
// @route   POST /api/v1/tournaments/test-register
// @access  Public (no auth required for testing)
router.post('/test-register', [
  body('ign').isLength({ min: 3, max: 20 }).withMessage('IGN must be between 3 and 20 characters'),
  body('uid').isLength({ min: 8, max: 12 }).isNumeric().withMessage('UID must be 8-12 digits'),
  body('whatsappNumber').isMobilePhone().withMessage('Invalid WhatsApp number'),
  body('countryCode').isLength({ min: 2, max: 4 }).withMessage('Invalid country code'),
  body('termsAccepted').isBoolean().withMessage('Terms acceptance is required')
], async (req, res, next) => {
  try {
    // Only allow in development/test mode
    if (process.env.NODE_ENV === 'production' && process.env.ALLOW_TEST_REGISTRATION !== 'true') {
      return res.status(403).json({
        success: false,
        error: 'Test registration is not available in production'
      });
    }
    ...
    const { ign, uid, whatsappNumber, countryCode, termsAccepted } = req.body;
    ...
    const testRegistrationId = `TEST_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

    // Log the test registration (in a real app, you might save to a test database)
    logger.info(`Test registration received:`, {
    ...
    res.status(200).json({
      ...
      data: {
        ...
        status: 'registered',
        tournament: {
          name: 'SR BIRD 1V1 FREE FIRE TOURNAMENT',
          type: 'Test Tournament',
          entryFee: 0,
          registeredAt: new Date().toISOString()
        },
        message: 'This is a test registration. No actual tournament entry has been created.'
      }
    });

  } catch (error) {
    logger.error('Test registration error:', error);
    next(error);
  }
});
