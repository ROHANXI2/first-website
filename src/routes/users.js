...
// @desc    Get all user registrations (admin function)
// @route   GET /api/v1/users/registrations
// @access  Public (frontend has its own password protection)
  ...
  try {
    ...
    const tournaments = await Tournament.find({})
      .populate({
        ...
        select: 'username gaming.freeFireName gaming.freeFireId profile.phoneNumber createdAt'
      })
      .select('participants title createdAt')
      .sort({ createdAt: -1 });

    const registrations = [];

    // Process each tournament's participants
    tournaments.forEach(tournament => {
      tournament.participants.forEach(participant => {
        if (participant.user) {
          const registrationDate = participant.registeredAt || participant.user.createdAt;

          registrations.push({
            date: registrationDate.toLocaleDateString('en-GB'),
            registrationTime: registrationDate.toLocaleTimeString('en-GB', {
              hour12: false,
              hour: '2-digit',
              minute: '2-digit'
            }),
            ign: participant.user.gaming?.freeFireName || 'N/A',
            uid: participant.user.gaming?.freeFireId || 'N/A',
            whatsapp: participant.user.profile?.phoneNumber || 'N/A',
            paymentStatus: participant.paymentStatus === 'completed' ? 'Paid' :
                          participant.paymentStatus === 'failed' ? 'Failed' : 'Pending',
            registrationStatus: participant.status === 'confirmed' ? 'Confirmed' :
                               participant.status === 'registered' ? 'Registered' :
                               participant.paymentStatus === 'failed' ? 'Payment Failed' : 'Pending',
            paymentMethod: 'UPI',
    ...
    registrations.sort((a, b) => new Date(b.date + ' ' + b.registrationTime) - new Date(a.date + ' ' + a.registrationTime));
