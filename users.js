...
// @desc    Get user registrations for admin dashboard
// @route   GET /api/v1/users/registrations
// @access  Private/Admin
router.get('/registrations', protect, authorize('admin'), async (req, res, next) => {
  try {
    // Get all users who have registered for tournaments (have freeFireId)
    const users = await User.find({
      'gaming.freeFireId': { $exists: true, $ne: null }
    }).sort({ createdAt: -1 });

    // Get payment data for these users
    ...
    payments.forEach(payment => {
      if (payment.user && payment.user._id) {
        paymentMap[payment.user._id.toString()] = payment;
      }
    });
    ...
    const registrations = users.map(user => {
      ...
      if (payment) {
        ...
        // For users without payment records (free registrations or test mode)
        paymentStatus = 'Free';
        registrationStatus = 'Confirmed';
      }
      ...
      return {
        ...
        ign: user.gaming.freeFireName || 'N/A',
        uid: user.gaming.freeFireId || 'N/A',
        whatsapp: user.profile.phoneNumber || 'N/A',
        paymentStatus,
        registrationStatus,
        paymentMethod,
        userId: user._id,
