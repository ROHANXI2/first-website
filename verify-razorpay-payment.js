  ...
  try {
    ...
    if (registration_data) {
      try {
        ...
        const { deviceId } = registration_data;

        // Check ban by Device ID (primary method)
        if (deviceId) {
          ...
          if (bans.length > 0) {
            ...
            if (banType === 'temporary' && banExpiryDate) {
              ...
              if (now > expiryDate) {
                ...
                console.log('Temporary ban expired during registration:', ban.id);
              } else {
                // Ban is still active
                return {
                  statusCode: 403,
                  ...
                  body: JSON.stringify({
                    success: false,
                    message: 'Registration denied - Device is banned',
              ...
              return {
                ...
                body: JSON.stringify({
                  success: false,
                  message: 'Registration denied - Device is permanently banned',
                  banReason: ban.get('Ban Reason'),
                  banType: banType,
                  deviceId: deviceId,
                  payment_id: razorpay_payment_id,
